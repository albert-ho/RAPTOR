...
transferTraffic() {
	...
	do
		i=$((i+1))
		scp -o ConnectTimeout=10 princeton_raptor@"$client":client.txt $data_path/parsed/client/client"$i".txt &
		scp -o ConnectTimeout=10 princeton_raptor@"$server":server.txt $data_path/parsed/server/server"$i".txt &
	done < nodes/client.txt 3< nodes/server.txt
}
