...
Assume this script has been downloaded on each PlanetLab Client machine.
...
the traffic incoming from the PlanetLab entry relay to the Client.
This will be in the format of each line being a packet's (time in seconds, bytes).
To do this, we need to determine which IP address it the Client machine's,
...
# Writes parsed traffic data from client.txt to client_parsed.txt
def write_client_parsed():
	node_ip = get_node_ip()
	tor_ip = get_tor_ip()
	f1 = open('client.txt', 'r')
	f2 = open('client_parsed.txt', 'w+')
	for line in f1:
		parsed = parse_line(line)
		if parsed == None: continue
		if (tor_ip == parsed['send_ip'] and node_ip == parsed['recv_ip']):
